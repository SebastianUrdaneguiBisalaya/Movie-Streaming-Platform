...
export const MoviesComponent = () => {
  ...
  const fetchData = async (page: number): Promise<void> => {
    try {
      setIsFetching(true);
      const data = await get<MovieApiResponse>(`/discover/movie?include_adult=false&include_video=false&language=en-US&page=${page}&sort_by=popularity.desc`);
      setMovies(prevMovies => page === 1 ? data.results : [...prevMovies, ...data.results]);
      setIsFetching(false);
    } catch (error) {
      throw new Error(`Movies are incorrect ${error}`)
    }
  }
